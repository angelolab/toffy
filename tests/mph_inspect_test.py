...
def test_compute_intensities(mocker):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        mph_data = mph_inspect.compute_intensities(
            tmp_dir, ["fov-1"], mass, mass_start, mass_stop, bin_factor=20
        )
...
def test_visualize_intensity_data(mocker):
    ...
    bin_file_dir = good_path = os.path.join(Path(__file__).parents[1], "data", "combined")
    mass = 98
    mass_start = 97.5
    mass_stop = 98.5
    ...
    with pytest.raises(ValueError, match="Not all values given in list"):
        mph_inspect.visualize_intensity_data(
            bin_file_dir, mass, mass_start, mass_stop, bad_fov_list
        )
