...
class RunCallbacks:
    ...
    def plot_mph_metrics(self, mph_output_dir, **kwargs):
        """Plots mph metrics generated by the `generate_mph` callback

        Args:
            mph_output_dir (str): directory containing qc metric csv
            **kwargs (Dict[str, Any]):
        ...
        # filter kwargs
        valid_kwargs = ['regression', 'save_dir']
        viz_kwargs = {k: v for k, v in kwargs.items() if k in valid_kwargs}

        mph_df = combine_mph_metrics(mph_output_dir, return_data=True)
        visualize_mph(mph_df, **viz_kwargs)
...
class FovCallbacks:
    ...
    def generate_mph(self, mph_output_dir, panel: pd.DataFrame = None):
        ...
        compute_mph_metrics(
            ...
            fov=self.point_name,
            target=self.__panel['Target'],
            mass_start=self.__panel['Start'],
            mass_stop=self.__panel['Stop']
        )
