...
def convert_panel(panel_path):
    ...
    # read in file to avoid metadata contained in the first few lines
    ...
    # add example panel values to new toffy panel, not keeping any duplicates
    toffy_panel = pd.concat([toffy_panel, example_panel], ignore_index=True)
    toffy_panel = toffy_panel.drop_duplicates(subset=['Target'])
