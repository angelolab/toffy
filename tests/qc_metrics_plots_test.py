...
def test_call_violin_swarm():
    ...
    plotting_df = plotting_df.append(
        pd.DataFrame([["TMA1_fov1", "chan0", "TMA1", 0.5]], columns=plotting_df.columns)
    )

    # Test that file is created.
    with tempfile.TemporaryDirectory() as temp_dir:
        _ = qc_metrics_plots.call_violin_swarm_plot(
            plotting_df, fig_label="test123", figsize=(20, 3), fig_dir=temp_dir
        )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        _ = qc_metrics_plots.call_violin_swarm_plot(
            plotting_df, fig_label="test123", figsize=(20, 3), fig_dir=None
        )
...
def test_make_batch_effect_plot():
    ...
    with tempfile.TemporaryDirectory() as top_level_dir:
        ...
        qc_metrics_plots.make_batch_effect_plot(
            data_dir=data_dir,
            normal_tissues=["fov1"],
