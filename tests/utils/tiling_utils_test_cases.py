...
def mock_coreg_dict(f):
    @mock.patch(
        "toffy.settings.COREG_PARAM_BASELINE",
        {
...
_VERIFY_ALL_COORD_CASES = [
    ...
    ((40, 50), "micron", False),
    ((40, 40), "micron", True),
    ((-101, 851), "optical", False),
    ((-100, 850), "optical", True),
]
...
def generate_tiled_region_params(
    start_x_roi_1=50,
    start_y_roi_1=150,
    num_row_roi_1=4,
    num_col_roi_1=2,
...
def generate_tiled_region_cases(
    roi_coord_list,
    roi_name_list,
    roi_sizes,
...
class TiledRegionReadCases:
    def case_no_reentry_no_moly_param(self):
        return generate_tiled_region_cases(
            ...
            deepcopy(_TILED_REGION_ROI_SIZES),
        )
...
class RhombusCoordInputCases:
    def case_rectangle(self):
        ...
        for x_coord in _TMA_RHOMBUS_X_COORDS:
            actual_pairs.extend(
                [(x_coord, y_coord) for y_coord in reversed(np.arange(1000, 2500, 500))]
            )
            ...
            np.vstack(
                [
                    ...
                    [
                        ...
                        0,
                        0,
                    ],  # NOTE: error only if STRICTLY GREATER than dist_threshold
                    [0, 0, 0, 0, 0, 49.99],
                    [0, 0, 0, 0, 0, 35.0],
                ]
            ),
...
class MappingDuplicateCases:
    ...
    def case_at_most_one_duplicate(self):
        ...
        duplicate_list = [("R0C1", ("R0C1", "R1C0")), ("R1C2", ("R1C1", "R1C2"))]
    ...
    np.vstack(
        [
            ...
            [
                ...
                0,
                0,
            ],  # since it's the 2nd FOV mapped to R1C0, throws a duplicate error
...
def generate_sample_annot(check_dist, check_duplicates, check_mismatches):
    ...
    if check_dist is not None:
        annot += "The following mappings are placed more than %d microns apart:\n\n" % check_dist
    ...
    if check_mismatches:
        annot += "The following mappings have mismatched names:\n\n"
