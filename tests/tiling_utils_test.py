...
from toffy import settings, tiling_utils
from toffy.json_utils import read_json_file, write_json_file

from .utils import test_utils
from .utils import tiling_utils_test_cases as test_cases
...
def test_assign_metadata_vals():
    ...
    example_output_dict = {3: True, 4: 1, 5: {"hello": "world"}, 6: 3.14, 7: None}
    ...
    misc_utils.verify_same_elements(
        new_output_keys=list(new_output_dict.keys()), valid_keys=[1, 3, 4, 5, 6, 7]
    )
...
def test_read_tiling_param(monkeypatch):
    ...
    sample_tiling_param = tiling_utils.read_tiling_param(
        "Sample prompt: ", "Sample error message", lambda x: x == 1, dtype=int
    )
    ...
    sample_tiling_param = tiling_utils.read_tiling_param(
        "Sample prompt: ", "Sample error message", lambda x: x == "Y", dtype=str
    )
...
def test_save_coreg_params():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        tiling_utils.save_coreg_params(
            sample_coreg_params_first, os.path.join("..", "toffy", "coreg_params.json")
        )
        ...
        tiling_utils.save_coreg_params(
            sample_coreg_params_second, os.path.join("..", "toffy", "coreg_params.json")
        )
...
@parametrize_with_cases(
    ...
    cases=test_cases.TiledRegionReadCases,
    glob="*_no_moly_param",
)
def test_read_tiled_region_inputs(
    monkeypatch,
    fov_coords,
    fov_names,
    fov_sizes,
    ...
    tiling_utils.read_tiled_region_inputs(sample_fovs_list, sample_region_params)
...
@parametrize(
    "region_corners_file",
    [
        param("bad_region_corners.json", marks=file_missing_err),
...
@parametrize_with_cases(
    "fov_coords, fov_names, fov_sizes, user_inputs, base_param_values, full_param_set",
    cases=test_cases.TiledRegionReadCases,
    glob="*_with_moly_param",
)
@parametrize("moly_interval_val", [0, 1])
def test_set_tiled_region_params(
    monkeypatch,
    region_corners_file,
    fov_coords,
    fov_names,
...
@parametrize_with_cases(
    ...
    cases=test_cases.TiledRegionMolySettingCases,
)
@parametrize("randomize_setting", [["N", "N"], ["N", "Y"], ["Y", "Y"]])
def test_generate_tiled_region_fov_list_base(
    moly_path,
    moly_roi_setting,
    moly_interval_setting,
    moly_interval_value,
    moly_insert_indices,
    roi_1_end_pos,
    randomize_setting,
    ...
    sample_roi_fovs_list = test_utils.generate_sample_fovs_list(
        fov_coords=[(0, 0), (100, 100)],
        fov_names=["TheFirstROI", "TheSecond_ROI"],
        fov_sizes=[5, 10],
    )
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        fov_names = [fov["name"] for fov in fov_list["fovs"]]
...
def test_generate_tiled_region_fov_list_oob():
    ...
    sample_roi_fovs_list = test_utils.generate_sample_fovs_list(
        fov_coords=[(0, 0), (100, 100)],
        fov_names=["TheFirstROI", "TheSecondROI"],
        fov_sizes=[5, 10],
    )
...
@parametrize(
    "extra_coords,extra_names",
    [param([(1, 2)], ["TheSecondFOV"], marks=value_err), param([], [])],
)
@parametrize(
    "num_row,num_col",
    [param(1, 3, marks=value_err), param(3, 1, marks=value_err), param(3, 4)],
)
@parametrize(
    "tma_corners_file",
    [param("bad_path.json", marks=file_missing_err), param("sample_tma_corners.json")],
)
@parametrize_with_cases("coords, actual_pairs", cases=test_cases.RhombusCoordInputCases)
...
def test_generate_tma_fov_list(
    tma_corners_file, extra_coords, extra_names, num_row, num_col, coords, actual_pairs
):
    ...
    fov_coords = [
        ...
        astuple(top_right),
        astuple(bottom_left),
        astuple(bottom_right),
    ] + extra_coords
    fov_names = ["TheFirstFOV"] * 4 + extra_names

    sample_fovs_list = test_utils.generate_sample_fovs_list(
        fov_coords=fov_coords, fov_names=fov_names, fov_sizes=[5] * len(fov_coords)
    )
...
def test_assign_closest_fovs():
    ...
    manual_to_auto_map, manual_auto_dist = tiling_utils.assign_closest_fovs(
...
def test_update_mapping_display():
    ...
    change = {"old": "row50_col25", "new": "row0_col25"}
    ...
    change = {"old": "row0_col25", "new": "row50_col50"}
...
def test_remap_manual_to_auto_display():
    ...
    w_man = widgets.Dropdown(
        options=[afi for afi in sorted(list(manual_coords.keys()))], value="row0_col0"
    )

    # define the old and the new auto FOV to map to
    change = {"old": "row3_col3", "new": "row3_col6"}
...
@parametrize_with_cases(
    "manual_to_auto_map, manual_auto_dist, actual_bad_dist_list",
...
@parametrize_with_cases(
    "manual_to_auto_map, actual_duplicate_list", cases=test_cases.MappingDuplicateCases
)
...
@parametrize_with_cases(
    "manual_to_auto_map, actual_mismatch_list", cases=test_cases.MappingMismatchCases
)
...
def test_delete_tiled_region_fovs(randomize, moly):
    ...
    # NOTE: ignore Moly points
    fovs_not_deleted = [fov for fov in list(set(fov_names) - set(fovs_delete)) if fov != "MoQC"]
    misc_utils.verify_same_elements(
        fov_names_rectangles=rectangle_names_list, fovs_to_keep=fovs_not_deleted
    )
    misc_utils.verify_same_elements(
        fov_names_run_json=tiled_region_names_list, fovs_to_keep=fovs_not_deleted
    )
...
def test_tiled_region_interactive_remap(num_regions, randomize, moly):
    ...
    non_moly_fovs = [
        fov["name"] for fov in sample_tiled_region_fovs["fovs"] if fov["name"] != "MoQC"
    ]
...
def test_tma_interactive_remap():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        sample_manual_fovs = {
            "fovs": [
                {"name": "R1C1", "centerPointMicrons": {"x": 100, "y": 100}},
                {"name": "R2C2", "centerPointMicrons": {"x": 200, "y": 200}},
            ]
        }
        ...
        write_json_file(
            json_path=os.path.join("..", "toffy", "coreg_params.json"),
...
def test_remap_and_reorder_fovs(moly_path, randomize_setting, moly_insert, moly_interval):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        remapped_sample_fovs = tiling_utils.remap_and_reorder_fovs(
            manual_sample_fovs_copy,
            sample_mapping,
            os.path.join(td, moly_path),
            randomize_setting,
            moly_insert,
            moly_interval,
        )
        ...
        misc_utils.verify_same_elements(
            remapped_fov_names=[
                fov["name"] for fov in remapped_sample_fovs["fovs"] if fov["name"] != "MoQC"
            ],
        ...
        scrambled_coords = [
            (fov["centerPointMicrons"]["x"], fov["centerPointMicrons"]["y"])
            for fov in remapped_sample_fovs["fovs"]
            if fov["name"] != "MoQC"
        ]
        misc_utils.verify_same_elements(
            scrambled_fov_coords=scrambled_coords, actual_coords=manual_coords
        )
        ...
        if moly_insert:
            ...
            fov_names = [
                remapped_sample_fovs["fovs"][i]["name"]
