...
def test_stitch_images(mocker, tiled):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        data = load_utils.load_imgs_from_dir(
            os.path.join(tmpdir, stitched_dir), files=["Au_stitched.tiff"]
        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        image_stitching.stitch_images(
            tmpdir, test_dir, ["Au", "CD3"], img_sub_folder="sub_dir", tiled=tiled
        )
        assert sorted(io_utils.list_files(os.path.join(tmpdir, stitched_dir))) == sorted(
            ["Au_stitched.tiff", "CD3_stitched.tiff"]
        )
        ...
        if not tiled:
            image_stitching.stitch_images(
                tmpdir, channels=["Au", "CD3"], img_sub_folder="sub_dir", tiled=tiled
            )
            assert sorted(io_utils.list_files(os.path.join(tmpdir, stitched_dir))) == sorted(
                ["Au_stitched.tiff", "CD3_stitched.tiff"]
            )
